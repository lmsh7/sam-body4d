...
def save_mesh_results(
    ...
    for pid, person_output in enumerate(outputs):
        ...
        if "ply" in export_formats:
            tmesh.export(f"{save_dir}/{oid}/{frame_stem}.ply")
        if "glb" in export_formats:
            tmesh.export(f"{save_dir}/{oid}/{frame_stem}.glb", file_type="glb")

        focal_length = {'focal_length': person_output["focal_length"].item(), 'camera': [float(x) for x in person_output['pred_cam_t']]}
        with open(f"{focal_dir}/{oid}/{os.path.basename(image_path)[:-4]}.json", "w") as f:
...
def save_skeleton_results(
    ...
    for pid, person_output in enumerate(outputs):
        ...
        oid = id_current[pid]
        out_path = f"{skeleton_dir}/{oid}/{frame_name}.json"
