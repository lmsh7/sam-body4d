...
def _assign_gpu_devices(num_workers):
    ...
    GPU (n-1) — they run before/after diffusion, never concurrently.
    """
...
class OfflineApp:
    def __init__(self, config_path: str = os.path.join(ROOT, "configs", "body4d.yaml")):
        ...
        if self.CONFIG.completion.get('enable', False):
            self.diffusion_workers, self.depth_model, self.max_occ_len, self.depth_device, hmr_device = build_diffusion_vas_config(self.CONFIG)
        else:
            self.diffusion_workers, self.depth_model, self.max_occ_len = [], None, None
            hmr_device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")

        self.sam3_3d_body_model = build_sam3_3d_body_config(self.CONFIG, device=hmr_device)
    ...
    def _run_obj_diffusion_offline(worker, obj_id, modal_pixels, depth_pixels,
        ...
        for bi, (a, b) in enumerate(zip(masks, pred_amodal_masks)):
            ...
            if i == 0 and bi == 0:
                if ious[0] < 0.7:
                    obj_ratio = bbox_from_mask(b)
                else:
                    obj_ratio = bbox_from_mask(a)
        ...
        occ_dict_obj = [1 if ix > 0.7 else 0 for ix in ious]
        ...
        if start is not None and end is not None:
            start = max(0, start-2)
            end = min(modal_batch.shape[1]-1, end+2)
            idx_dict_obj = (start, end)
            ...
            idx_path_obj = {'images': completion_image_path, 'masks': completion_masks_path}
            ...
            # --- RGB completion ---
            completion_image_path = idx_path_obj['images']
            ...
            for img in pred_amodal_rgb_save:
                cv2.imwrite(os.path.join(completion_image_path, f"{idx_:08d}.jpg"),
        ...
        return obj_id, {
            'obj_ratio': obj_ratio,
            'occ_dict': occ_dict_obj,
            'idx_dict': idx_dict_obj,
            'idx_path': idx_path_obj,
        }
    ...
    def on_4d_generation(self, video_path: str=None):
        ...
        MASKS_PATH = os.path.join(self.OUTPUT_DIR, 'masks')
        ...
        for obj_id in self.RUNTIME['out_obj_ids']:
            ...
            if self.RUNTIME['smpl_export']:
                os.makedirs(f"{self.OUTPUT_DIR}/skeleton_4d_individual/{obj_id}", exist_ok=True)
        ...
        modal_pixels_list = []
        if len(self.diffusion_workers) > 0:
            for obj_id in self.RUNTIME['out_obj_ids']:
        ...
        obj_ratio_dict = {}
        num_workers = len(self.diffusion_workers)

        # Initialize rendering backend
        _render_backend = self.CONFIG.get("sam_3d_body", {}).get("rendering", {}).get("backend", "pyrender")
        _mesh_cfg = self.CONFIG.get("sam_3d_body", {}).get("mesh_export", {})
        ...
        for i in tqdm(range(0, n, batch_size)):
            ...
            if len(modal_pixels_list) > 0:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
                    for wi, (modal_pixels, obj_id) in enumerate(zip(modal_pixels_list, self.RUNTIME['out_obj_ids'])):
                        worker = self.diffusion_workers[wi % num_workers]
                        fut = executor.submit(
                            self._run_obj_diffusion_offline,
            ...
            mask_outputs, id_batch, empty_frame_list = process_image_with_mask(self.sam3_3d_body_model, batch_images, batch_masks, idx_path, idx_dict, mhr_shape_scale_dict, occ_dict, cam_int=cam_int, inference_type=_inference_type, global_obj_ids=self.RUNTIME['out_obj_ids'])
            ...
            if gpu_renderer is not None:
                ...
                def _save_io(frame_idx):
                    ...
                    cv2.imwrite(f"{self.OUTPUT_DIR}/rendered_frames/{frame_stem}.jpg", combined_imgs[frame_idx])
                    ...
                    for pi, pimg in enumerate(individual_imgs[frame_idx]):
                        ...
                        cv2.imwrite(f"{self.OUTPUT_DIR}/rendered_frames_individual/{oid}/{frame_stem}_{oid}.jpg", pimg)
                    ...
                    if self.RUNTIME['smpl_export'] and outputs_by_frame[frame_idx] is not None:
                        save_skeleton_results(
                            outputs=outputs_by_frame[frame_idx],
                            skeleton_dir=f"{self.OUTPUT_DIR}/skeleton_4d_individual",
                            image_path=image_path,
                            id_current=ids_by_frame[frame_idx],
                        )
                ...
                def _render_and_save(args):
                    ...
                    for ri, rend_img in enumerate(rend_img_list):
                        ...
                        cv2.imwrite(
                            f"{self.OUTPUT_DIR}/rendered_frames_individual/{oid}/{os.path.basename(image_path)[:-4]}_{oid}.jpg",
                            rend_img.astype(np.uint8),
                        )
        ...
        if self.RUNTIME['smpl_export']:
            aggregate_skeleton_npz(f"{self.OUTPUT_DIR}/skeleton_4d_individual")

        if _mesh_export_enabled and _animated_glb:
            aggregate_mesh_glb(f"{self.OUTPUT_DIR}/mesh_4d_individual", fps=25.0)
